...
public:
    ...
    void job_set(int n,int s, char *b);
};
...
void job::job_set(int n, int s, char *b)
{
    sockfd = s;
    number = n;
    strcpy(buf, b);
...
{
    int ran;
    char str[103]="abcdefghijklmnopqrstuvwxyzQWERTYUIOPASDFGHJKLZXCVBNM0123456789";
    for(int i=0; i<10; i++)
    {
        ran = rand()%62;
        buf[i] = str[ran];
...
void threadpools :: create_threads()
{
   ...
   // q = Pid;
    ...
    pthread_create(&Pid[i], NULL, fun, NULL);//创建线程池
   // q -> next = p;
   // p -> next = NULL;
   // q = p;
}
